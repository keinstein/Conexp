...
public class AttributeIncrementalImplicationCalculator
	...
	public void calcImplications() {
		...
		final AttributeInformationSupplier ais = m_isBasis.getAttributesInformation();
		...
		m_isConceptsAndBasis.addImplication(new ImplicationEx(msEntireObjectSet, m_sEmptyAttributeSet, m_sEmptyAttributeSet));
	...
	private void addAttribute(final int nAttributeNumber) {
		m_isBasis.clear();
		...
		final Set sAttributeExtent = getAttributeExtent(nAttributeNumber);
	...
	private boolean computeSaturation(final Implication iImplication, final ImplicationSet isExtraImplications) {
		final ImplicationSet isAllImplications = new ImplicationSet(m_isBasis.getAttributesInformation());
		isAllImplications.addAll(m_isBasis);
		isAllImplications.addAll(isExtraImplications);
		
		final ModifiableSet msPremise = iImplication.getModifiablePremise();
		final Set sConclusion = iImplication.getConclusion();
	...
	private void processStableConcept(final Set sAttributeExtent, final int nAttributeNumber, final ImplicationEx ieConcept) {
		//Assert.isTrue(ieConcept.isConcept());
		final ModifiableSet msNewExtent = ieConcept.getExtent().makeModifiableSetCopy();
		msNewExtent.and(sAttributeExtent);
		final ModifiableSet msNewPremise = ieConcept.getPremise().makeModifiableSetCopy();
		msNewPremise.put(nAttributeNumber);
		
		final ModifiableSet msNewConclusion = getIntent(msNewExtent);
	...
	private ModifiableSet getIntent(final Set sExtent) {
		final ModifiableSet msIntent = ContextFactoryRegistry.createSet(rel.getColCount());
		msIntent.fill();
	...
	private void processStableImplication(final Implication iImplication) {
		m_isBasis.addImplication(iImplication);
	}

	private boolean processModifiedConcept(final ImplicationEx ieConcept, final int nAttributeNumber, final int nConceptIndex) {
		//Assert.isTrue(m_isConceptsAndBasis.getImplication(nConceptIndex) == ieConcept);
		//Assert.isTrue(ieConcept.getConclusion().isEmpty());
		
		final Set sIntent = ieConcept.getPremise();
		boolean fNewImplication = IsMinimalPremise(sIntent, m_isMinModImpl);
		if (fNewImplication) {
			final ImplicationEx ieNewImplication = new ImplicationEx(ieConcept.getExtent(), sIntent, m_sEmptyAttributeSet);
	...
	private boolean IsMinimalPremise(final Set sPremise, final ImplicationSet isImplications) {
		final int nSize = isImplications.getSize();
	...
	private boolean processModifiedImplication(final ImplicationEx ieImplication, final int nAttributeNumber, final int nImplicationIndex) {
	...
	private Set getAttributeExtent(int nAttributeNumber) {
		final int nObjectCount = rel.getRowCount();
		final ModifiableSet msExtent = ContextFactoryRegistry.createSet(nObjectCount);
	...
	private static final ConceptImplicationComparator CIC = new ConceptImplicationComparator();
